...
@pytest.mark.asyncio
async def test_no_comment_column_found():
    """Test error handling when no valid comment column is found"""
    # Create temp directory and file
    temp_dir = tempfile.mkdtemp()
    project_dir = os.path.join(temp_dir, "test_project")
    os.makedirs(project_dir, exist_ok=True)
    ...
    # Create test CSV with no text columns
    df = pd.DataFrame({
        "id": [1, 2, 3],
    ...
    try:
        ...
        with patch('rag.has_rag_dependencies', True), \
            ...
            assert "No valid comment column found" in str(excinfo.value)
...
@pytest.mark.asyncio
async def test_process_comment_with_rag_empty_comment():
    """Test processing an empty comment with full pipeline"""
    # Test with empty string and full processing pipeline
    with patch('rag.find_similar_comments') as mock_find_similar, \
         patch('rag.classify_comment_with_llm') as mock_classify:
        ...
        # Should return default values without calling find_similar or classify
